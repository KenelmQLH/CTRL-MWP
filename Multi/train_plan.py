...
def train():
    ...
    for fold in range(4, 5):
        ...
        # train
        solver.zero_grad()
        log = open(f'result_{DATA_NAME}/log_' + str(fold) + '.txt', 'w')
        for e in range(epochs):
            ...
            if (e > 0 and e % 10 == 0) or epochs - e < 5:
                ...
                f = open(f'result_{DATA_NAME}/correct_result_dev_' + str(fold) + '.jsonl', 'w')
                for d in correct_results:
                    json.dump(d, f, ensure_ascii=False)
                    f.write("\n")
                f.close()
                f = open(f'result_{DATA_NAME}/wrong_results_dev_' + str(fold) + '.jsonl', 'w')
                for d in wrong_results:
                    json.dump(d, f, ensure_ascii=False)
                    f.write("\n")
                f.close()
                f = open(f'result_{DATA_NAME}/all_results_dev_' + str(fold) + '.jsonl', 'w')
                ...
                solver.save_pretrained(f'result_{DATA_NAME}/fold_' + str(fold) +'/models')
                tokenizer.save_pretrained(f'result_{DATA_NAME}/fold_' + str(fold) +'/models')
        log.close()
